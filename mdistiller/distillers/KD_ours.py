...
def spearman(logits_student, logits_teacher):
    sr = soft_rank(logits_student)
    tr = soft_rank(logits_teacher)

    x_bar = sr.mean(dim=1, keepdim=True)
    y_bar = tr.mean(dim=1, keepdim=True)

    cov = ((sr - x_bar) * (tr - y_bar)).sum(dim=1)
    var_x = (sr - x_bar).pow(2).sum(dim=1)
    var_y = (tr - y_bar).pow(2).sum(dim=1)

    # 1e-3 (not 1e-8) to prevent gradient explosion when var_x * var_y → 0
    return - cov / torch.sqrt(var_x * var_y + 1e-3)
...
class SDD_KD(Distiller):
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        # Spearman loss (unweighted)
        wspm_per_sample = spearman(logits_student / self.temperature, logits_teacher / self.temperature)
        loss_wspm = self.wspm_loss_weight * wspm_per_sample.mean()
        losses_dict["loss_wspm"] = loss_wspm
